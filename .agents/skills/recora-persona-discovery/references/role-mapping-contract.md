...
`decision_role` is the detailed role. `role_type` is the canonical downstream role bucket. Do not swap them.

## Required Fields

| field | meaning | example |
|---|---|---|
| `decision_role` | Detailed decision or influence role inferred from the business type and evidence. | `procurement`, `security_reviewer`, `local_comparator` |
...
- Keep `decision_role` specific enough to explain the search intent.
- Keep `role_type` within the canonical role values used by `prompt-angle-handoff-contract.md`.
- Always include `role_mapping_reason` in handoff rows. Use `same_as_decision_role` when `decision_role` and `role_type` are identical.

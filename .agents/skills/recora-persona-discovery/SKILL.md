...
- `references/output-mode-contract.md`: output modes for full, excerpt, handoff-only, and validation-only outputs.
...
- `references/role-mapping-contract.md`: mapping between detailed `decision_role` values and canonical `role_type` values.
...
- `output_mode`: `full`, `excerpt`, `handoff_only`, or `validation_only`. If missing, infer the smallest mode that satisfies the user's request and state it.
...
Use `references/role-mapping-contract.md` when producing tables or handoff rows. Keep `decision_role` as the detailed role, keep `role_type` as the canonical downstream role bucket, and add `role_mapping_reason` when they differ.
...
- Detailed `decision_role` and canonical `role_type` are not mixed, and `role_mapping_reason` is present.

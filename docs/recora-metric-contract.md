...
- 顧客向け名称は、画面とドキュメントで揃えます。`lib/recora/metric-definitions.ts` の `recoraDisplayMetricLabels` をUI側の表示名の正本にします。
...
AI回答ページで一部だけ表示する場合は、「全1,200件中 1-50件を表示」のように表示範囲として説明します。この文言は `formatRecoraDisplayRange` で生成し、50件や100件を別のKPIとして扱いません。

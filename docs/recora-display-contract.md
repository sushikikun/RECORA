...
- 「通算」は使わず、「全体」または「継続モニタリング」を使う。
- 「インサイト」は使わず、「改善候補」を使う。
- 「根拠確認」はレポート配下に置く。
- トピック/プロンプトは初期リリースでは「新しい測定」内に置く。
- レポート一覧と実行履歴は分ける。
...
3. 「全体」配下に概要/推移を作る。
4. レポート一覧と実行履歴を分離する。
5. レポート配下にレポート概要、測定条件、AI回答、ブランド比較、参照元、根拠確認、改善候補を配置する。
6. 「インサイト」を「改善候補」に置き換える。
7. トピック/プロンプトは「新しい測定」内に置く。
8. 実行履歴には failed / partial / QA / aggregate run を含める。

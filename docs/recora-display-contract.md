...
- レポート一覧は常時表示する。
- レポート概要 / AI回答 / ブランド比較 / 参照元 / 改善候補は、選択中reportIdがある時だけ表示する。
- レポート詳細タブは、過去レポート一覧でレポートを選択した後に出す。
- 「インサイト」は使わず、「改善候補」を使う。
...
4. ホーム、レポート一覧、選択レポート詳細、実行履歴を分離する。
5. レポート一覧は常時表示し、選択中reportIdがある時だけレポート概要、AI回答、ブランド比較、参照元、改善候補を配置する。
...
- /dashboard/reports/[id] 配下でレポート詳細タブと実行履歴が出るか

...
2026-08-01: Issue #122 P4-B PR #127 remains Draft / Human review on `codex/issue-122-p4b-account-access`. The revision covers provider_fixture plus paired operator evidence compatibility, `email_confirmed_at`-only recipient verification, the customer-session P4-A legacy inventory gate, current operator replay authorization and target binding, immutable temporal replay results, and the tracked Phase 3 verifier. Required isolated reset, integration, negative, concurrency, temporal replay, DTO, P4-A/Phase 3 regression, advisor, lint, preflight, typecheck, lint, build, dashboard read-model, diff, scope, secret, environment, DB URL, lockfile, and commit-gate checks are recorded in the PR before handoff. `npm run recora:commit-check` may stop only at the repository's generic migration auto-allow guard for the OWNER-approved additive migration. P4-C #123 / PR #126 is separately Execute-approved and remains Draft / Human review; P4-D / #124 remains unstarted and unapproved.

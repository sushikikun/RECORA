...
This Stage 1 baseline authorized no schema/migration, Auth/signup/invitation implementation, RLS, DB/Supabase, provider/webhook/API, package/lockfile, deploy, child-Issue, or Stage 2 Execute work. Later OWNER comments separately authorize P4-B Issue #122 and P4-C Issue #123. P4-B is limited to one current additive migration, server-only wrapper, verifier, and documentation updates; P4-C is limited to its own function-only additive migration, server-only module, verifier, and Exec Plan. P4-D / #124 remains unstarted and unapproved.
...
| Invitation lifecycle | P4-A one-time record/history: `pending`, `accepted`, `expired`, `revoked`, `superseded`. | Operator `invite.create` creates pending; bound recipient `invite.accept` accepts; clock marks expiry; operator `invite.revoke` revokes; resend creates a new pending invitation and supersedes the old one. | Accept after expiry/revocation/supersession; cancel after acceptance; cross-tenant or mismatched-recipient acceptance; revive terminal row. | Invitation ID + command type; acceptance binds verified recipient. | No direct grant; accepted invitation is only membership input. |
...
Acceptance is tenant-bound and one-time. Replay returns the immutable recorded outcome only for the same verified identity and command key; it never creates another membership. Expired pending invitations are finalized with an `expired` event before a same-recipient reinvite. Live resend creates a new invitation ID, transitions the old pending row to `superseded`, and records `superseded_by_invitation_id` plus the append-only superseded event. Expired, revoked, superseded, canceled, and cross-tenant attempts fail closed. Revoked membership and ended/canceled contract episodes are not silently resurrected: a later authorized relationship uses a new invitation, membership episode, contract event, or contract episode. Event, snapshot, audit, and lifecycle evidence remains append-only.

P4-B PR #127 implements customer invitation accept as an authenticated-only `SECURITY DEFINER` RPC that derives actor identity from `auth.uid()`, requires `auth.users.email_confirmed_at` (not generic `confirmed_at`), normalizes the confirmed email in the database, hashes it with SHA-256, and compares it to the tenant-bound invitation recipient binding. Customer accept command receipts use `customer_session` actor evidence and pass the P4-A legacy-inventory gate; operator create/resend/revoke/membership commands remain service-role-only and require current identity, permission, scope, and operator audit evidence. Replays reconstruct the original command result from append-only evidence even after later state changes.
The customer-safe TypeScript access DTO is a minimal projection: it exposes only `entitlement.capabilityAllowed`, never raw capability or limit maps and never an unapproved fixed product-key set. P4-C-shaped safe technical keys such as `measurement`, `analysis`, and `prompts` resolve through the injected server RPC boundary; provider, billing, audit, pointer, internal, raw, payload, secret, token, credential, authorization, and private namespaces fail closed. After replay authorization succeeds, pending/state/expiry/recipient and membership-state rejection paths revalidate current operator identity, permission, scope, and the actual invitation or membership ID, then write one `denied` audit event with the command reason, failure reason, request ID, and correlation ID. They do not create an accepted P4 receipt, operator command receipt, current-row change, append-only domain event, or membership episode mutation. Unexpected operator receipt failures retain a `failed` audit event.

The one approved P4-B actor-evidence exception is confined to the current additive migration: add enum value `p4_source_kind = customer_session`, add nullable `recora_private.p4_command_receipts.customer_auth_user_id uuid references auth.users(id) on delete restrict`, and replace `p4_command_receipt_actor_shape`. That constraint permits a provider fixture with either no actor evidence or a complete operator audit/command-receipt pair, while rejecting customer evidence and half-pairs. The existing P4-A `superseded` state and `superseded_by_invitation_id` integrity rules are consumed as-is; no P4-A migration is edited.
...
- P4-B Issue #122 and P4-C Issue #123 are separately Execute-authorized and remain Draft PRs / Human review. P4-D / #124 remains unstarted and unapproved. This document itself creates no child Issue, product implementation, Auth/DB/Supabase/provider action, package change, deploy, or UI decision.

## 10. P4-B PR #127 validation record

2026-08-01: Issue #122 P4-B PR #127 remains Draft / Human review on `codex/issue-122-p4b-account-access`. OWNER comment `5151116159` is covered by the P4-C-shaped `measurement`/`analysis`/`prompts` entitlement integration, minimal `capabilityAllowed` DTO projection, exact post-authorization denied audit for pending/state/expiry/recipient/membership and cross-tenant rejection, and no accepted receipt/domain mutation on rejection. The revision also preserves provider_fixture plus paired operator evidence compatibility, `email_confirmed_at`-only recipient verification, the customer-session P4-A legacy inventory gate, current operator replay authorization and target binding, immutable temporal replay results, and the tracked Phase 3 verifier. Required isolated migration-only/seeded resets, integration, negative, concurrency, temporal replay, DTO, P4-A/Phase 3 regression, advisor, lint, preflight, typecheck, lint, build, dashboard read-model, diff, scope, secret, environment, DB URL, lockfile, and commit-gate checks are recorded before handoff. P4-C #123 / PR #126 is separately Execute-approved and remains Draft / Human review; P4-D / #124 remains unstarted and unapproved.

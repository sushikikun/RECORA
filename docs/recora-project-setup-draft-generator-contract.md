...
- Customer-facing question-area labels, topic names, diagnosis goals, expected signals, and prompt-support labels should be normalized to Japanese for `ja` seeds; internal enum/type names remain internal.
- `finance_investment` should only receive strong category boost when explicit finance evidence appears in service description, category, or audience text. Generic BtoC price/cost wording must not route school, clinic, EC, local, or professional-service inputs into finance.
- Generator regression fixtures are stored as UTF-8 file-based verification fixtures under `scripts/fixtures/**` to avoid PowerShell inline Japanese mojibake.

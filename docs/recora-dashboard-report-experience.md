...
- non-branded scopeを安全に作れない場合は、全観測へfallbackせず、visibility/ranking/SOV/competitor gapを未集計または0件スコープとして扱う。
- 判定できない場合は無理に断定せず、ranking系の比較スコープには混ぜない。必要な確認事項として扱う。
...
## Phase 1 Admin Demo Update

Phase 1 report pages use the centralized report ready gate before treating a project URL as customer-ready.

- Overview blocks customer-facing report content unless the gate returns `customer_ready`.
- Detailed report tabs should avoid rendering unfinished or placeholder-backed data when the gate is missing or not customer-ready.
- Unknown data origin, demo/local/sample evidence, and placeholder-backed read models fail closed for customer-facing routes.
- `/dashboard` and `/dashboard/reports` use `RECORA_DEFAULT_PROJECT_SLUG` when set. If it is not set, they should not silently fall back to a demo project.
- Recommendation lists use customer-visible quality-gated recommendations only. Pre-quality-gate candidates remain administrator review material.
- Prompt type and metric eligibility remain a documented contract until schema support exists. Branded prompts are sentiment-only and do not feed visibility/ranking/SOV/competitor-gap metrics.

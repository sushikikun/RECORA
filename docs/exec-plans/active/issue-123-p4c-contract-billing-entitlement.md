...
> Latest OWNER blocking authority: [5150556826](https://github.com/sushikikun/RECORA/issues/123#issuecomment-5150556826)
...
items in `5150078684`, and the recovery/retry/transitive-supersession blockers in `5150556826`. P4-A migration,
...
- `public.recora_p4c_record_lifecycle_checkpoint_attempt_command` records
  ...
  after exhaustion without creating duplicate receipts, snapshots, checkpoints,
  or outbox effects.
...
Completed on 2026-08-01 against latest local head after OWNER `5150556826`:
...
- Issue #123 full matrix passed after the final retry/receipt-order fix: `RECORA_ISSUE_123_DB_CONTAINER=supabase_db_recoraissue123 RECORA_ISSUE_123_SUPABASE_WORKDIR=C:/tmp/recora-issue-123-p4c-supabase npx tsx scripts/verify-issue-123-p4c-contract-billing-entitlement.ts`.
- The Issue #123 matrix includes provider-neutral fixture checks; billing receipt/payment fact projection; immutable entitlement snapshot/pointer derivation; pending checkpoint/outbox creation; recovery from `reconciliation_required` and exhausted checkpoints by later correct Phase 3 success evidence; retryable failure, too-early failed-to-pending retry without an accepted receipt, later same-retry success after `next_attempt_at`, attempt count, retry exhaustion, exhaustion recovery, replay/conflict/concurrency, rollback, double snapshot/effect prevention, and customer access recovery.
...
- `npm run recora:preflight:full` passed. Local warnings only: `.env.local` missing, mixed migration naming style, known mock/static dashboard fallback refs, uncommitted/important-area changes for the approved 4 files, and LF/CRLF notices.

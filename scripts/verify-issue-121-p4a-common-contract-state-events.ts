...
begin
  ...
  foreach relation_name in array array[
    ...
    'p4_billing_receipt_events','p4_normalized_payment_facts','p4_downstream_checkpoints','p4_durable_outbox'
  ] loop
  ...
  insert into recora_private.p4_downstream_checkpoints(organization_id,command_receipt_id,required_effect,state,stable_reason)
  values(org_id,command_id,'lifecycle.effect','pending','checkpoint_pending');
  ...
  begin insert into recora_private.p4_invitations(organization_id,recipient_binding_hash,state,issuer_command_receipt_id,last_command_receipt_id,request_id,correlation_id,expires_at) values(org_id,repeat('d',64),'accepted',command_id,command_id,gen_random_uuid(),gen_random_uuid(),now()+interval '1 day'); raise exception 'P4 invitation accepted without recipient proof'; exception when check_violation then null; end;

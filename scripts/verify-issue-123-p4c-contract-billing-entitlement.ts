...
assert.doesNotMatch(moduleSqlBoundary, /blocksCustomerAccess:\\s*boolean|resolvedDocument:\\s*|currentContractState:\\s*|existingReceipt:\\s*|latestSourceSequence:\\s*/);
...
from public.recora_p4c_apply_contract_billing_entitlement_command(
  ...
  ${sqlText(args.p_authoritative_plan_policy_key as string)},
  ...
  ${sqlText(args.p_payload_fingerprint as string | null)},
  ${sqlUuid(args.p_corrects_payment_fact_id as string | null)},
  ${sqlText(args.p_downstream_effect_result as string)}
) result_row;
commit;
`;
}
...
const rpcClient = {
  async rpc(name: "recora_p4c_apply_contract_billing_entitlement_command", args: Record<string, unknown>) {
...
values ('12360000-0000-4000-8000-000000000001'::uuid, ${sqlText(POLICY_KEY)}, 1, now() - interval '2 days', ${sqlJson({ capabilities: { measurement: true }, limits: { prompts: 6 } })});
...
values ('12360000-0000-4000-8000-000000000002'::uuid, ${sqlText(POLICY_KEY)}, 1, now() - interval '1 day', ${sqlJson({ capabilities: { measurement: true, analysis: true }, limits: { prompts: 12 } })}, '12360000-0000-4000-8000-000000000001'::uuid);
commit;
`;
}
function createOperatorEvidence(orgId: string, projectId: string, sequence: number): OperatorEvidence {
...
function makeCommand(input: {
  ...
  downstreamEffectResult?: Phase4ProviderNeutralBillingCommand["downstreamEffectResult"];
  ...
  const evidence = createOperatorEvidence(orgId, projectId, evidenceSequence);
  return {
    ...
    authoritativePlanPolicyKey: POLICY_KEY,
    ...
    downstreamEffectResult: input.downstreamEffectResult ?? "pending"
  };
}
...
function normalizeProcessResult(rows: unknown[]): Phase4CustomerSafeContractResult {
  ...
  const result: Phase4CustomerSafeContractResult = {
    schemaVersion: p4c.phase4ContractBillingIntegrationSchemaVersion,
    outcome: String(row.outcome) as Phase4CustomerSafeContractResult["outcome"],
    stableReason: String(row.stable_reason),
    customerAccessAllowed: Boolean(row.customer_access_allowed),
    reasonCode: String(row.reason_code) as Phase4CustomerSafeContractResult["reasonCode"],
...
async function seedActiveContract(orgId: string, projectId: string, baseSequence: number, contractReference: string): Promise<Phase4CustomerSafeContractResult> {
  assert.equal((await execute(makeCommand({ orgId, projectId, sequence: baseSequence, contractReference, sourceReference: `${contractReference}.receipt.draft`, idempotencyKey: `${contractReference}.draft`, nextContractState: "draft" }))).customerAccessAllowed, false);
  assert.equal((await execute(makeCommand({ orgId, projectId, sequence: baseSequence + 1, contractReference, sourceReference: `${contractReference}.receipt.pending`, idempotencyKey: `${contractReference}.pending`, nextContractState: "pending_activation" }))).customerAccessAllowed, false);
  return execute(makeCommand({ orgId, projectId, sequence: baseSequence + 2, contractReference, sourceReference: `${contractReference}.receipt.activate`, idempotencyKey: `${contractReference}.activate`, nextContractState: "active", paymentFactKind: "payment_succeeded" }));
}
...
do $verify$
begin
  if to_regprocedure('public.recora_p4c_apply_contract_billing_entitlement_command(uuid,uuid,recora_private.p4_source_kind,text,text,bigint,text,recora_private.p4_contract_state,recora_private.p4_payment_fact_kind,text,text,text,uuid,uuid,uuid,uuid,text,uuid,text)') is null then
  ...
  if has_function_privilege('anon', 'public.recora_p4c_apply_contract_billing_entitlement_command(uuid,uuid,recora_private.p4_source_kind,text,text,bigint,text,recora_private.p4_contract_state,recora_private.p4_payment_fact_kind,text,text,text,uuid,uuid,uuid,uuid,text,uuid,text)', 'EXECUTE')
    or has_function_privilege('authenticated', 'public.recora_p4c_apply_contract_billing_entitlement_command(uuid,uuid,recora_private.p4_source_kind,text,text,bigint,text,recora_private.p4_contract_state,recora_private.p4_payment_fact_kind,text,text,text,uuid,uuid,uuid,uuid,text,uuid,text)', 'EXECUTE')
  then
    raise exception 'Issue 123 P4-C RPC is executable by a browser role';
  end if;
  if not has_function_privilege('service_role', 'public.recora_p4c_apply_contract_billing_entitlement_command(uuid,uuid,recora_private.p4_source_kind,text,text,bigint,text,recora_private.p4_contract_state,recora_private.p4_payment_fact_kind,text,text,text,uuid,uuid,uuid,uuid,text,uuid,text)', 'EXECUTE') then
...
begin;
set local role anon;
select * from public.recora_p4c_apply_contract_billing_entitlement_command(${sqlUuid(ORG_A)},${sqlUuid(PROJECT_A)},'provider_fixture'::recora_private.p4_source_kind,'fixture.p4c','receipt.denied',1,'contract.denied','draft'::recora_private.p4_contract_state,'payment_unknown'::recora_private.p4_payment_fact_kind,'chain.denied',${sqlText(POLICY_KEY)},'p4c.denied',${sqlUuid(requestId(1))},${sqlUuid(correlationId(1))},${sqlUuid(OPERATOR_ID)},${sqlUuid(OPERATOR_ID)},null,null,'pending');
rollback;
`, /permission denied/i);
  queryLocal(`
begin;
set local role authenticated;
select * from public.recora_p4c_apply_contract_billing_entitlement_command(${sqlUuid(ORG_A)},${sqlUuid(PROJECT_A)},'provider_fixture'::recora_private.p4_source_kind,'fixture.p4c','receipt.denied.auth',2,'contract.denied','draft'::recora_private.p4_contract_state,'payment_unknown'::recora_private.p4_payment_fact_kind,'chain.denied',${sqlText(POLICY_KEY)},'p4c.denied.auth',${sqlUuid(requestId(2))},${sqlUuid(correlationId(2))},${sqlUuid(OPERATOR_ID)},${sqlUuid(OPERATOR_ID)},null,null,'pending');
rollback;
`, /permission denied/i);
  ...
  assert.equal(p4c.isProviderNeutralBillingEnvelope({ ...active, capabilities: { measurement: false } }), false);
  assert.equal(p4c.isProviderNeutralBillingEnvelope({ ...active, blocksCustomerAccess: false }), false);
  assert.match(p4c.createPhase4ContractBillingPayloadFingerprint(active), /^[0-9a-f]{64}$/);
...
`);
  ...
  assert.equal(staleResult.outcome, "reconciliation_required");
  ...
  const mismatch = makeCommand({ sequence: 111, sourceReference: "receipt.mismatch", idempotencyKey: "p4c.mismatch", nextContractState: "paused", paymentFactKind: "payment_failed", payloadFingerprint: "a".repeat(64) });
  const mismatchBefore = domainCounts(ORG_A);
  const mismatchResult = await execute(mismatch);
  assert.equal(mismatchResult.outcome, "rejected");
  assert.equal(mismatchResult.stableReason, "invalid_reference");
  assert.equal(mismatchResult.customerAccessAllowed, false);
  assertNoDomainChange(mismatchBefore, domainCounts(ORG_A), ["commandReceipts", "billingReceipts", "paymentFacts", "contractEvents", "snapshots", "checkpoints", "outbox", "projectionVersion"]);
  ...
  const pendingCheckpoint = makeCommand({ orgId: ORG_C, projectId: PROJECT_C, sequence: 404, contractReference: "contract.pending", sourceReference: "receipt.pending.pause", idempotencyKey: "p4c.pending.pause", nextContractState: "paused", paymentFactKind: "payment_failed", downstreamEffectResult: "pending" });
  ...
  const pauseRecon = makeCommand({ sequence: 105, sourceReference: "receipt.pause.recon", idempotencyKey: "p4c.pause.recon", nextContractState: "paused", paymentFactKind: "payment_failed", downstreamEffectResult: "reconciliation_required" });
  const pauseResult = await execute(pauseRecon);
  assert.equal(pauseResult.outcome, "reconciliation_required");
  assert.equal(pauseResult.customerAccessAllowed, false);
  assert.equal(pauseResult.reasonCode, "reconciliation_required");
  assert.equal(countValue(`select count(*) from recora_private.p4_downstream_checkpoints where organization_id = ${sqlUuid(ORG_A)} and project_id = ${sqlUuid(PROJECT_A)} and blocks_customer_access and superseded_by_checkpoint_id is null and state = 'reconciliation_required'`), 1);

  const recovery = makeCommand({ sequence: 106, sourceReference: "receipt.recover", idempotencyKey: "p4c.recover", nextContractState: "active", paymentFactKind: "payment_succeeded", downstreamEffectResult: "completed" });
  ...
  assert.equal(countValue(`select count(*) from recora_private.p4_downstream_checkpoints where organization_id = ${sqlUuid(ORG_A)} and project_id = ${sqlUuid(PROJECT_A)} and blocks_customer_access and superseded_by_checkpoint_id is null and state in ('pending','applying','failed','reconciliation_required')`), 0);
  assert.equal(countValue(`select count(*) from recora_private.p4_durable_outbox where organization_id = ${sqlUuid(ORG_A)} and project_id = ${sqlUuid(PROJECT_A)} and state = 'delivered' and correction_of_outbox_id is not null`), 1);

  const rollbackBefore = domainCounts(ORG_A);
  const rollbackFailure = makeCommand({ sequence: 107, sourceReference: "receipt.rollback", idempotencyKey: "p4c.rollback", nextContractState: "paused", paymentFactKind: "payment_reversed", downstreamEffectResult: "completed", correctsPaymentFactId: "12399999-0000-4000-8000-000000000999" });
  ...
  assert.deepEqual(domainCounts(ORG_A), rollbackBefore);

...
import { updateRecoraPassword } from "./actions";
...
export default function UpdatePasswordPage({ searchParams = {} }: UpdatePasswordPageProps) {
  const message = getUpdatePasswordMessage(searchParams);
      ...
      <div className="mx-auto flex min-h-[calc(100vh-4rem)] w-full max-w-4xl items-center">
        <section className="grid w-full overflow-hidden rounded-lg border border-[#DDE8E5] bg-white shadow-[0_24px_70px_rgba(15,23,42,0.10)] md:grid-cols-[0.9fr_1.1fr]">
          ...
          <div className="px-6 py-7 sm:px-8 sm:py-9">
            ...
            <form action={updateRecoraPassword} className="mt-6 space-y-5">
              <input autoComplete="username" className="hidden" name="username" type="text" />
              ...
              <label className="block">
                <span className="text-sm font-bold text-slate-700">新しいパスワード確認</span>
                <span className="mt-2 flex h-12 items-center gap-2 rounded-md border border-[#DDE8E5] bg-white px-4 transition focus-within:border-[#00796B] focus-within:ring-2 focus-within:ring-[#00796B]/15">
                  <LockKeyhole className="h-4 w-4 shrink-0 text-slate-400" />
                  <input
                    className="h-full min-w-0 flex-1 bg-transparent text-base text-slate-950 outline-none placeholder:text-slate-400"
...
function getUpdatePasswordMessage(searchParams: UpdatePasswordPageProps["searchParams"]): AuthMessageData | null {
  ...
  if (searchParams?.error) {
    return {
      tone: "error",
      text: "パスワードを更新できませんでした。再設定メールを開き直してから再度お試しください。"
    };
  }

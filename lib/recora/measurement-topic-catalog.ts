...
const AUDIENCE_PACKS = new Set<RecoraTopicPackKeyV3>([
  "b2b_buying",
  "b2c_decision"
]);
...
const INDUSTRY_PACKS: readonly RecoraTopicPackKeyV3[] = [
...
const STRUCTURE_SIGNAL_BY_PACK: Partial<
  Record<RecoraTopicPackKeyV3, readonly RecoraGenerationStructureSignal[]>
> = {
  agency_delivery: ["agency_delivery"],
  b2b2c: ["b2b2c"],
  marketplace_demand: ["marketplace_brand"],
  marketplace_supply: ["marketplace_brand"],
  ...
  public_nonprofit: ["public_nonprofit_customer"],
  enterprise_it_security: ["enterprise_it_security"],
  healthcare_clinic: ["adult_healthcare"],
  care_welfare: ["care_welfare"],
  education_school: [
    "adult_education",
    "child_education",
    "corporate_training"
...
const DOMAIN_BY_PACK: Partial<
  Record<RecoraTopicPackKeyV3, RecoraBusinessDomain>
> = {
  enterprise_it_security: "it_software",
  healthcare_clinic: "healthcare",
  care_welfare: "care_welfare",
  education_school: "education",
  professional_service: "professional_consulting",
  recruiting_hr: "recruiting_hr",
  real_estate: "real_estate",
  finance_insurance: "finance_insurance",
  travel_hospitality: "travel_hospitality",
  restaurant_food_catering: "food_beauty_lifestyle",
  beauty_wellness: "food_beauty_lifestyle",
  construction_home_service: "construction_home_service",
  ...
  media_content_advertising: "media_content",
  publisher_content: "media_content"
};

const MARKET_SIDES_BY_PACK: Partial<
  Record<RecoraTopicPackKeyV3, readonly RecoraGenerationCustomerSide[]>
> = {
...
function tierForPack(
  ...
  if (INDUSTRY_PACKS.includes(pack)) return "industry";
...
function policyApplicability(
  pack: RecoraTopicPackKeyV3
): RecoraTopicBlueprintApplicabilityV3 {
  const value = { ...EMPTY_APPLICABILITY };
  if (pack === "b2b_buying") value.audienceScopesAny = ["b2b", "both"];
  if (pack === "b2c_decision" || pack === "family_proxy") {
    value.audienceScopesAny = ["b2c", "both"];
  }
  if (pack === "both_audience") value.audienceScopesAny = ["both"];

  const signals = STRUCTURE_SIGNAL_BY_PACK[pack];
  if (signals) value.structureSignalsAny = signals;

  const domain = DOMAIN_BY_PACK[pack];
  if (domain) value.primaryBusinessDomainsAny = [domain];

  if (pack === "restaurant_food_catering") {
    value.resolvedTopicSubtypeKeysAny = ["food_dining"];
  }
  if (pack === "beauty_wellness") {
    value.resolvedTopicSubtypeKeysAny = ["beauty_wellness"];
  }
  return value;
}
...
function makePackPolicies(): readonly RecoraTopicPackPolicyV3[] {
  return RECORA_TOPIC_PACK_KEYS.map((pack) => {
    const rows = RECORA_TOPIC_ALL_SOURCE_ROWS_V3.filter(
      (row) => row[0] === pack
    );
    const lanes = Array.from(new Set(rows.map((row) => row[5])));
    ...
    return {
      ...
      defaultMarketSidesAny: MARKET_SIDES_BY_PACK[pack] ?? null,
...
function mergeApplicability(
  ...
  override?: Partial<RecoraTopicBlueprintApplicabilityV3>
): RecoraTopicBlueprintApplicabilityV3 {
...
function coverageDimensions(
  ...
  lane: RecoraTopicMeasurementLaneKeyV3,
  tier: RecoraTopicSpecificityTierV3,
  ...
  if (primary === "T3" && family === "fit_and_selection") {
    values.push("T1");
  }
  ...
  if (primary === "T6") {
    values.push("T3");
    if (lane === "trust_risk_diagnostic") values.push("T5");
  }
...
export function validateRecoraMeasurementTopicCatalogV3() {
  ...
  for (const item of catalog) {
    const signature = stableJson({
      goal: item.measurementGoal,
      entities: item.expectedEntityTypes,
      axes: item.comparisonAxes,
  ...
  for (const item of catalog) {
    const signature = stableJson({
      tier: item.specificityTier,
      semantic: item.semanticGroupKey,
      applicability: item.applicability
    });
    const previous = semantic.get(signature);
    if (previous) {
      blockers.push(
        `same_tier_semantic_overlap:${previous}:${item.blueprintKey}`
      );
    } else {
      semantic.set(signature, item.blueprintKey);
    }
  }
  ...
  const minimum: Record<RecoraTopicCoverageDimensionV3, number> = {
    T1: 45,
    T2: 45,
    T3: 55,
    T4: 45,

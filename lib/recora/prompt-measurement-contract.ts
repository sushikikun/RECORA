...
export type RecoraPromptRevisionContractCandidate = Omit<
  ...
  "promptRevisionId" | "promptVersion" | "intentCellId" | "contentHash"
> & {
...
export function validateIntentCellContract(
  value: RecoraIntentCellContract
  ...
  if (
    new Set(value.secondaryTopicIds).size !== value.secondaryTopicIds.length ||
    value.secondaryTopicIds.includes(value.primaryTopicId)
  ) {
  ...
  if (value.status === "active" && !value.trackingScope) {
    blockers.push("active_intent_cell_requires_tracking_scope");
  }
...
export function validatePromptRevisionContract(
  ...
  validateContractVersion(blockers, value.contractVersion);
  if (!Number.isInteger(value.promptVersion) || value.promptVersion < 1) {
    blockers.push("prompt_version_invalid");
  }
  ...
  if (value.lifecycleStatus === "active") {
    if (!hasAnyEligibleMetric(value.metricEligibility)) {
      blockers.push("active_revision_requires_eligible_analysis");
    }
    if (value.metricEligibilityAuthority !== "explicit_contract") {
      blockers.push("active_revision_requires_explicit_eligibility");
    }
    if (!["provided", "legacy_explicit"].includes(value.sourceStatus)) {
      blockers.push("active_revision_requires_explicit_source_status");
    }
    if (
      value.qualityScoreSource !== "calculated" ||
      value.qualityScore == null
    ) {
      blockers.push("active_revision_requires_calculated_quality");
    }
    if (!hasText(value.effectiveFrom)) {
      blockers.push("active_revision_effective_from_missing");
    }
    if (value.metricEligibility.reasons.length === 0) {
      blockers.push("active_revision_eligibility_reasons_missing");
    }
    if (value.promptVersion > 1 && !hasText(value.supersedesPromptRevisionId)) {
      blockers.push("active_revision_version_requires_supersedes");
    }
  }
...
export function validatePromptSetVersionContract(
  ...
  if (productionStatus) {
    ...
    if (
      !getRecoraPromptProfileDefinition(value.profileId)
        .productionMeasurementEligible
    ) {
      blockers.push("production_set_requires_measurement_profile");
    }
  }
  if (value.status === "frozen" && !hasText(value.frozenAt)) {
    blockers.push("frozen_at_missing");
  }
  if (
    !productionStatus &&
    !getRecoraPromptProfileDefinition(value.profileId)
...
export function validatePromptSetCompilationContract(
  ...
  for (const duplicate of duplicateValues(
    value.intentCells.map((item) => item.intentCellId)
  )) {
    blockers.push(`duplicate_intent_cell_id:${duplicate}`);
  }
  for (const duplicate of duplicateValues(
  ...
  const productionStatus = ["active", "frozen"].includes(
    value.promptSetVersion.status
  );
  ...
  for (const membership of value.memberships) {
    ...
    if (
      ...
      blockers.push(`membership_set_version_mismatch:${membership.membershipId}`);
    }

    const intentCell = intentCells.get(membership.intentCellId);
    const revision = revisions.get(membership.promptRevisionId);
    if (!intentCell) {
      blockers.push(`membership_unknown_intent_cell:${membership.membershipId}`);
    }
    if (!revision) {
      blockers.push(`membership_unknown_prompt_revision:${membership.membershipId}`);
    }
    if (revision) {
      if (revision.intentCellId !== membership.intentCellId) {
        blockers.push(`membership_revision_intent_mismatch:${membership.membershipId}`);
      }
      if (revision.variantRole !== membership.variantRole) {
        blockers.push(`membership_revision_variant_mismatch:${membership.membershipId}`);
      }
      if (productionStatus && revision.lifecycleStatus !== "active") {
        blockers.push(`production_membership_revision_not_active:${membership.membershipId}`);
      }
    }
    if (productionStatus && intentCell && intentCell.status !== "active") {
      blockers.push(`production_membership_intent_cell_not_active:${membership.membershipId}`);
    }

    if (membership.panelRole === "core") {
      roleCounts.core += 1;
      coreIntentCells.add(membership.intentCellId);
      coreCountByIntentCell.set(
        membership.intentCellId,
        (coreCountByIntentCell.get(membership.intentCellId) ?? 0) + 1
      );
    }
  ...
  for (const [intentCellId, count] of coreCountByIntentCell) {
    if (count !== 1) {
      blockers.push(`core_canonical_count_invalid:${intentCellId}`);
    }
  }
  for (const intentCellId of robustnessIntentCells) {
    if (!coreIntentCells.has(intentCellId)) {
      blockers.push(`robustness_without_core_intent_cell:${intentCellId}`);
    }
  }

  if (productionStatus) {
    const profile = getRecoraPromptProfileDefinition(
      value.promptSetVersion.profileId
    );

...
export const RECORA_INTENT_CELL_STATUSES = ["candidate", "active", "held", "retired", "rejected", "superseded"] as const;
...
export type RecoraIntentCellStatus = typeof RECORA_INTENT_CELL_STATUSES[number];
...
export type RecoraIntentCellContract = {
  contractVersion: ContractVersion;
  intentCellId: string;
  projectId: string;
  primaryTopicId: string;
  secondaryTopicIds: readonly string[];
  personaId: string;
...
export type RecoraPromptRevisionContractCandidate = Omit<RecoraPromptRevisionContract, "promptRevisionId" | "promptVersion" | "intentCellId" | "contentHash"> & {
...
export type LegacyPromptDraftAdapterContext = RecoraPromptRevisionIdentityContext & {
...
export type RecoraPromptSetCompilationContract = {
  ...
  intentCells: readonly RecoraIntentCellContract[];
...
export function createExcludedPromptMetricEligibility(reasons: readonly string[] = []): RecoraPromptMetricEligibility {
  return {
    visibility: "excluded",
    ranking: "excluded",
    sov: "excluded",
...
export function getRecoraPromptProfileDefinition(id: RecoraPromptProfileId): RecoraPromptProfileDefinition {
  const definition = RECORA_PROMPT_PROFILE_DEFINITIONS.find((item) => item.id === id);
...
export function validateIntentCellContract(value: RecoraIntentCellContract): RecoraContractValidationResult {
  ...
  if (value.expectedSignalTypes.length === 0) blockers.push("expected_signal_types_missing");
...
export function validatePromptRevisionContract(value: RecoraPromptRevisionContract): RecoraContractValidationResult {
  ...
  if (value.brandScope === "named_comparison" && value.questionAct !== "compare_candidates") blockers.push("named_comparison_requires_compare_act");
  if (marketEligible && value.brandScope !== "brand_excluded") blockers.push("market_metrics_require_brand_excluded_scope");
  if (marketEligible && ["medium", "high"].includes(value.seedContaminationRisk)) blockers.push("market_metrics_reject_seed_contamination");
  if (value.metricEligibility.visibility === "eligible" && (!supportsMarketMetricResponseShape(value.responseShape) || !["direct", "likely"].includes(value.candidateMentionOpportunity))) blockers.push("visibility_eligibility_invalid");
  if (value.metricEligibility.ranking === "eligible" && (!supportsMarketMetricResponseShape(value.responseShape) || !["direct", "comparable_set"].includes(value.rankingOpportunity))) blockers.push("ranking_eligibility_invalid");
  ...
  if (["evaluation_criteria", "explanatory_answer", "evidence_answer", "branded_sentiment_answer"].includes(value.responseShape) && marketEligible) blockers.push("non_market_response_shape_in_market_metrics");
  ...
  if (forcedCitation && marketEligible) blockers.push("forced_citation_prompt_in_market_metrics");
  if ((forcedCitation && value.metricEligibility.forcedCitationValidation !== "eligible") || (!forcedCitation && value.metricEligibility.forcedCitationValidation === "eligible")) blockers.push("forced_citation_eligibility_invalid");
  ...
  if (value.lifecycleStatus === "active") {
    if (!hasAnyEligibleMetric(value.metricEligibility)) blockers.push("active_revision_requires_eligible_analysis");
    ...
    if (!hasText(value.effectiveFrom)) blockers.push("active_revision_effective_from_missing");
    if (value.metricEligibility.reasons.length === 0) blockers.push("active_revision_eligibility_reasons_missing");
    if (value.promptVersion > 1 && !hasText(value.supersedesPromptRevisionId)) blockers.push("active_revision_version_requires_supersedes");
  }
...
export function validatePromptRevisionIdentityContext(value: RecoraPromptRevisionContract, context: RecoraPromptRevisionIdentityContext): RecoraContractValidationResult {
  const blockers: string[] = [];
  const warnings: string[] = [];
  const normalizedText = normalizeIdentity(value.text);
  const targetSignals = identitySignals(context.brandIdentity);
  const competitorSignals = uniqueStrings([...(context.knownCompetitors ?? []), ...(context.knownCompetitorAliases ?? [])]).map(normalizeIdentity).filter((signal) => signal.length >= 2);
  const targetPresent = targetSignals.some((signal) => normalizedText.includes(signal));
  const competitorPresent = competitorSignals.some((signal) => normalizedText.includes(signal));
  if (targetSignals.length === 0) warnings.push("target_brand_identity_context_missing");
  if (["competitor_only", "named_comparison"].includes(value.brandScope) && competitorSignals.length === 0) blockers.push("known_competitor_identity_context_missing");
  if (value.brandScope === "brand_excluded" && targetPresent) blockers.push("target_brand_signal_in_brand_excluded_text");
  if (value.brandScope === "self_branded" && !targetPresent) blockers.push("self_branded_text_missing_target_brand");
  if (value.brandScope === "competitor_only" && targetPresent) blockers.push("competitor_only_contains_target_brand");
  if (value.brandScope === "competitor_only" && competitorSignals.length > 0 && !competitorPresent) blockers.push("competitor_only_text_missing_known_competitor");
  if (value.brandScope === "named_comparison" && !targetPresent && !competitorPresent) blockers.push("named_comparison_text_missing_named_entity");
  if (hasMarketMetricEligibility(value.metricEligibility) && competitorPresent) blockers.push("known_competitor_signal_in_market_prompt");
  return validationResult(blockers, warnings);
}
...
export function validatePromptSetMembershipContract(value: RecoraPromptSetMembershipContract): RecoraContractValidationResult {
  ...
  if (!hasText(value.inclusionReason)) warnings.push("inclusion_reason_missing");
...
export function validatePromptSetVersionContract(value: RecoraPromptSetVersionContract): RecoraContractValidationResult {
  ...
  const warnings: string[] = [];
  const productionStatus = ["active", "frozen"].includes(value.status);
  validateContractVersion(blockers, value.contractVersion);
  if (productionStatus) {
    blockers.push(...requiredFields([
      ...
      ["repeat_policy_id", value.repeatPolicyId],
  ...
  if (value.status === "frozen" && !hasText(value.frozenAt)) blockers.push("frozen_at_missing");
  if (!productionStatus && !getRecoraPromptProfileDefinition(value.profileId).productionMeasurementEligible) warnings.push("design_preview_profile");
  return validationResult(blockers, warnings);
}
...
export function validateExecutionProfileContract(value: RecoraExecutionProfileContract): RecoraContractValidationResult {
  ...
  if (new Set(value.domainFilters).size !== value.domainFilters.length) blockers.push("duplicate_domain_filter");
  return validationResult(blockers, warnings);
}

export function validatePromptSetCompilationContract(value: RecoraPromptSetCompilationContract): RecoraContractValidationResult {
  const blockers: string[] = [];
  const warnings: string[] = [];
  validateContractVersion(blockers, value.contractVersion);
  appendValidation(blockers, warnings, "set_version", value.promptSetVersion.promptSetVersionId, validatePromptSetVersionContract(value.promptSetVersion));
  const intentCells = new Map<string, RecoraIntentCellContract>();
  const revisions = new Map<string, RecoraPromptRevisionContract>();
  for (const duplicate of duplicateValues(value.intentCells.map((item) => item.intentCellId))) blockers.push(`duplicate_intent_cell_id:${duplicate}`);
  for (const duplicate of duplicateValues(value.promptRevisions.map((item) => item.promptRevisionId))) blockers.push(`duplicate_prompt_revision_id:${duplicate}`);
  for (const duplicate of duplicateValues(value.memberships.map((item) => item.membershipId))) blockers.push(`duplicate_membership_id:${duplicate}`);
  for (const duplicate of duplicateValues(value.memberships.map((item) => item.promptRevisionId))) blockers.push(`duplicate_prompt_revision_membership:${duplicate}`);
  for (const duplicate of duplicateValues(value.memberships.map((item) => String(item.sortOrder)))) blockers.push(`duplicate_sort_order:${duplicate}`);
  for (const intentCell of value.intentCells) {
    intentCells.set(intentCell.intentCellId, intentCell);
    appendValidation(blockers, warnings, "intent_cell", intentCell.intentCellId, validateIntentCellContract(intentCell));
  }
  for (const revision of value.promptRevisions) {
    revisions.set(revision.promptRevisionId, revision);
    appendValidation(blockers, warnings, "prompt_revision", revision.promptRevisionId, validatePromptRevisionContract(revision));
  }
  const coreCountByIntentCell = new Map<string, number>();
  const coreIntentCells = new Set<string>();
  const robustnessIntentCells = new Set<string>();
  const roleCounts: Record<"core" | "robustness" | "diagnostic", number> = { core: 0, robustness: 0, diagnostic: 0 };
  const productionStatus = ["active", "frozen"].includes(value.promptSetVersion.status);
  for (const membership of value.memberships) {
    appendValidation(blockers, warnings, "membership", membership.membershipId, validatePromptSetMembershipContract(membership));
    if (membership.promptSetVersionId !== value.promptSetVersion.promptSetVersionId) blockers.push(`membership_set_version_mismatch:${membership.membershipId}`);
    const intentCell = intentCells.get(membership.intentCellId);
    const revision = revisions.get(membership.promptRevisionId);
    if (!intentCell) blockers.push(`membership_unknown_intent_cell:${membership.membershipId}`);
    if (!revision) blockers.push(`membership_unknown_prompt_revision:${membership.membershipId}`);
    if (revision) {
      if (revision.intentCellId !== membership.intentCellId) blockers.push(`membership_revision_intent_mismatch:${membership.membershipId}`);
      if (revision.variantRole !== membership.variantRole) blockers.push(`membership_revision_variant_mismatch:${membership.membershipId}`);
      if (productionStatus && revision.lifecycleStatus !== "active") blockers.push(`production_membership_revision_not_active:${membership.membershipId}`);
    }
    if (productionStatus && intentCell && intentCell.status !== "active") blockers.push(`production_membership_intent_cell_not_active:${membership.membershipId}`);
    if (membership.panelRole === "core") {
      roleCounts.core += 1;
      coreIntentCells.add(membership.intentCellId);
      coreCountByIntentCell.set(membership.intentCellId, (coreCountByIntentCell.get(membership.intentCellId) ?? 0) + 1);
    }
    if (membership.panelRole === "robustness") {
      roleCounts.robustness += 1;
      robustnessIntentCells.add(membership.intentCellId);
    }
    if (membership.panelRole === "diagnostic") roleCounts.diagnostic += 1;
  }
  for (const [intentCellId, count] of Array.from(coreCountByIntentCell.entries())) {
    if (count !== 1) blockers.push(`core_canonical_count_invalid:${intentCellId}`);
  }
  for (const intentCellId of Array.from(robustnessIntentCells.values())) {
    if (!coreIntentCells.has(intentCellId)) blockers.push(`robustness_without_core_intent_cell:${intentCellId}`);
  }
  if (productionStatus) {
    const profile = getRecoraPromptProfileDefinition(value.promptSetVersion.profileId);
    if (profile.coreCanonical !== roleCounts.core) blockers.push("profile_core_count_mismatch");
    if (profile.robustness !== roleCounts.robustness) blockers.push("profile_robustness_count_mismatch");
    if (profile.diagnostic !== roleCounts.diagnostic) blockers.push("profile_diagnostic_count_mismatch");
    if (roleCounts.core + roleCounts.robustness + roleCounts.diagnostic !== profile.targetTotal) blockers.push("profile_base_total_mismatch");
  }
  return validationResult(blockers, warnings);
}
...
export function adaptLegacyPromptDraftToContractCandidate(prompt: PromptDraft, context: LegacyPromptDraftAdapterContext): RecoraCompatibilityAdapterResult<RecoraPromptRevisionContractCandidate> {
  const warnings = ["legacy_prompt_requires_intent_cell_and_revision", "legacy_eligibility_is_proposed"];
  const reviewReasons: string[] = [];
  const missingFields = ["promptRevisionId", "promptVersion", "intentCellId", "contentHash", "calculatedQualityScore"];
  if (!hasText(prompt.promptId)) {
    reviewReasons.push("legacy_prompt_id_missing");
    missingFields.push("promptId");
  }
  const brandScope = deriveLegacyBrandScope(prompt, context, warnings, reviewReasons);
  const questionFamily = deriveLegacyQuestionFamily(prompt, context.topicType, reviewReasons);
  ...
  const value: RecoraPromptRevisionContractCandidate = {
    ...
    metricEligibility: deriveLegacyMetricEligibility(prompt, brandScope, questionFamily, questionAct),
    metricEligibilityAuthority: "compatibility_inferred",
  ...
  const status = brandScope === "brand_optional" || prompt.gateDecision === "reject" ? "blocked" : reviewReasons.length > 0 ? "manual_review" : "needs_contract_fields";
  return { status, value, missingFields: uniqueStrings(missingFields), warnings: uniqueStrings(warnings), reviewReasons: uniqueStrings(reviewReasons) };
}
...
export function adaptLegacyPromptScopeToContractCandidate(scope: RecoraPromptScope): RecoraCompatibilityAdapterResult<RecoraLegacyPromptScopeCandidate> {
  ...
  return {
    status: reviewReasons.length > 0 ? "manual_review" : "needs_contract_fields",
    value: {
      contractVersion: RECORA_PROMPT_MEASUREMENT_CONTRACT_VERSION,
      ...
      metricEligibility: createExcludedPromptMetricEligibility(["legacy_scope_insufficient"]),
      metricEligibilityAuthority: "compatibility_inferred"
    },
    missingFields: ["promptText", "promptRevisionId", "intentCellId", "responseShape", "opportunityFields"],
    warnings: ["legacy_scope_never_confers_official_eligibility"],
    reviewReasons
  };
}
...
export function projectPromptRevisionToLegacyScope(revision: RecoraPromptRevisionContract, primaryPurpose: RecoraMeasurementPurpose | null = null): RecoraLegacyScopeProjectionResult {
  ...
  if (!validation.valid) return blockedLegacyProjection(["target_contract_invalid", ...validation.blockers]);
  if (revision.lifecycleStatus !== "active" || revision.metricEligibilityAuthority !== "explicit_contract") return blockedLegacyProjection(["projection_requires_active_explicit_revision"]);
  const promptType = projectLegacyPromptType(revision);
  const representablePurposes = getRepresentableLegacyPurposes(revision.metricEligibility);
  const warnings = ["lossy_projection"];
  const genericMarketMismatch = promptType === "comparison_generic" && hasMarketMetricEligibility(revision.metricEligibility);
  ...
  if (measurementPurpose && !representablePurposes.includes(measurementPurpose)) {
  ...
  if (genericMarketMismatch && measurementPurpose && ["visibility", "ranking", "sov"].includes(measurementPurpose)) {
    measurementPurpose = null;
  ...
  if (measurementPurpose && representablePurposes.length > 1) warnings.push("other_eligible_purposes_omitted");
  return { status: "projected", scope: { promptType, measurementPurpose, status: "explicit", notes: warnings }, warnings };
}

function profile(id: RecoraPromptProfileId, targetTotal: number, coreCanonical: number | null = null, robustness: number | null = null, diagnostic: number | null = null): RecoraPromptProfileDefinition {
  const productionMeasurementEligible = coreCanonical != null;
  return { id, kind: productionMeasurementEligible ? "experimental_measurement" : "design_preview", targetTotal, coreCanonical, robustness, diagnostic, productionMeasurementEligible, experimental: productionMeasurementEligible };
}

function deriveLegacyBrandScope(prompt: PromptDraft, context: LegacyPromptDraftAdapterContext, warnings: string[], reviewReasons: string[]): RecoraPromptBrandScope {
  ...
  const targetPresent = targetSignals.some((signal) => normalizedText.includes(signal));
  const competitorPresent = competitorSignals.some((signal) => normalizedText.includes(signal));
  const declaredSelfBranded = prompt.brandingMode === "branded" || prompt.brandMentionRule === "brand_included";
  const declaredBrandExcluded = prompt.brandingMode === "non_branded" && prompt.brandMentionRule === "brand_excluded";
  const declaredNamedCompetitor = prompt.competitorMentionRule === "named_competitors";
  if (declaredSelfBranded && !targetPresent) reviewReasons.push("brand_included_metadata_without_target_signal");
  if (declaredBrandExcluded && targetPresent) reviewReasons.push("target_brand_signal_in_brand_excluded_prompt");
  if (prompt.competitorMentionRule === "no_competitor" && competitorPresent) reviewReasons.push("known_competitor_signal_with_no_competitor_policy");
  if (declaredNamedCompetitor && competitorSignals.length === 0) warnings.push("named_competitor_identity_missing");
  ...
  if (competitorPresent || declaredNamedCompetitor || prompt.brandingMode === "competitor_only" || prompt.brandMentionRule === "competitor_only") {
...
function deriveLegacyQuestionFamily(prompt: PromptDraft, topicType: TopicType | null | undefined, reviewReasons: string[]): RecoraQuestionFamily {
  ...
  const categoryMap: Partial<Record<PromptDraft["category"], RecoraQuestionFamily>> = {
  ...
  if (prompt.category === "persona_based") {
    if (prompt.intentType === "risk_checking") return "implementation_operation";
    reviewReasons.push("persona_family_review");
  }
  if (prompt.intent === "problem_aware") return "problem_solution";
  if (["candidate_list", "ranked_recommendation"].includes(prompt.responseShape)) return "category_discovery";
...
function deriveLegacyMetricEligibility(prompt: PromptDraft, brandScope: RecoraPromptBrandScope, questionFamily: RecoraQuestionFamily, questionAct: RecoraQuestionAct): RecoraPromptMetricEligibility {
  if (prompt.gateDecision === "reject") return createExcludedPromptMetricEligibility(["rejected"]);
  const forcedCitation = questionAct === "request_sources" || questionFamily === "citation_evidence" || prompt.responseShape === "evidence_answer";
  const marketBase = brandScope === "brand_excluded" && !forcedCitation && supportsMarketMetricResponseShape(prompt.responseShape) && ["none", "low"].includes(prompt.seedContaminationRisk);
  ...
  const sentiment = brandScope === "self_branded" && (questionFamily === "branded_perception" || prompt.responseShape === "branded_sentiment_answer");
  const riskCheck = prompt.intentType === "risk_checking" || ["regulated_risk", "implementation_operation"].includes(questionFamily);
  const recommendationInput = visibility || ranking || sentiment || forcedCitation || riskCheck || ["ask_evaluation_criteria", "ask_explanation", "compare_candidates"].includes(questionAct);
  return {
    ...
    naturalCitationObservation: metricState(!forcedCitation && brandScope !== "brand_optional"),
...
function deriveLegacyVariantRole(prompt: PromptDraft): RecoraPromptVariantRole {
  return prompt.category === "citation_check" || prompt.category === "branded" || prompt.brandingMode !== "non_branded" ? "diagnostic" : "canonical";
}

function legacyPromptTypeToBrandScope(promptType: RecoraPromptType | null): RecoraPromptBrandScope | null {
...
function legacyPromptTypeToQuestionFamily(promptType: RecoraPromptType | null): RecoraQuestionFamily | null {
  if (promptType === "branded") return "branded_perception";
  if (["comparison_generic", "comparison_named", "competitor_named"].includes(promptType ?? "")) return "competitor_comparison";
  if (promptType === "citation_check") return "citation_evidence";
  if (promptType === "non_branded") return "market_discovery";
  return null;
}

function projectLegacyPromptType(revision: RecoraPromptRevisionContract): RecoraPromptType {
  ...
  if (revision.questionFamily === "competitor_comparison" || revision.questionAct === "compare_candidates") return "comparison_generic";
  return "non_branded";
}

function getRepresentableLegacyPurposes(value: RecoraPromptMetricEligibility): RecoraMeasurementPurpose[] {
  ...
  return pairs.filter(([key]) => value[key] === "eligible").map(([, purpose]) => purpose);
}

function identitySignals(value: BrandIdentityForDraft): string[] {
  return uniqueStrings([value.brandName, value.serviceName, value.domain, value.officialSiteUrl ? extractHostname(value.officialSiteUrl) : null, ...(value.aliases ?? [])]).map(normalizeIdentity).filter((signal) => signal.length >= 2);
}

function blockedLegacyProjection(warnings: string[]): RecoraLegacyScopeProjectionResult {
  return { status: "blocked", scope: { promptType: null, measurementPurpose: null, status: "missing", notes: warnings }, warnings };
}

function isForcedCitationRevision(value: RecoraPromptRevisionContract): boolean {
  return value.questionAct === "request_sources" || value.questionFamily === "citation_evidence" || value.responseShape === "evidence_answer";
}

function hasMarketMetricEligibility(value: RecoraPromptMetricEligibility): boolean {
  return [value.visibility, value.ranking, value.sov].includes("eligible");
}

function hasAnyEligibleMetric(value: RecoraPromptMetricEligibility): boolean {
  return RECORA_PROMPT_METRIC_KEYS.some((key) => value[key] === "eligible");
}

function supportsMarketMetricResponseShape(value: PromptResponseShape): boolean {
  return ["candidate_list", "ranked_recommendation", "comparative_set"].includes(value);
}
...
function validateRevisionDates(value: RecoraPromptRevisionContract, blockers: string[], warnings: string[]): void {
  const effectiveFrom = parseTimestamp(value.effectiveFrom);
  const effectiveTo = parseTimestamp(value.effectiveTo);
  if (value.effectiveFrom && effectiveFrom == null) blockers.push("effective_from_invalid");
  if (value.effectiveTo && effectiveTo == null) blockers.push("effective_to_invalid");
  if (effectiveFrom != null && effectiveTo != null && effectiveTo <= effectiveFrom) blockers.push("effective_interval_invalid");
  if (value.supersedesPromptRevisionId === value.promptRevisionId && hasText(value.supersedesPromptRevisionId)) blockers.push("prompt_revision_cannot_supersede_itself");
  if (value.lifecycleStatus === "superseded" && !hasText(value.effectiveTo)) warnings.push("superseded_revision_effective_to_missing");
}

function validateScore(blockers: string[], value: number | null): void {
  if (value != null && (!Number.isFinite(value) || value < 0 || value > 100)) blockers.push("score_out_of_range");
}

function requiredFields(entries: readonly (readonly [string, string | null | undefined])[]): string[] {
  return entries.filter(([, value]) => !hasText(value)).map(([name]) => `${name}_missing`);
}

function validateContractVersion(blockers: string[], value: string): void {
  if (value !== RECORA_PROMPT_MEASUREMENT_CONTRACT_VERSION) blockers.push("contract_version_mismatch");
}

function appendValidation(blockers: string[], warnings: string[], entityType: string, entityId: string, result: RecoraContractValidationResult): void {
  for (const blocker of result.blockers) blockers.push(`${entityType}:${entityId}:${blocker}`);
  for (const warning of result.warnings) warnings.push(`${entityType}:${entityId}:${warning}`);
}

function validationResult(blockers: string[], warnings: string[]): RecoraContractValidationResult {
  const uniqueBlockers = uniqueStrings(blockers);
  const uniqueWarnings = uniqueStrings(warnings);
  return { valid: uniqueBlockers.length === 0, blockers: uniqueBlockers, warnings: uniqueWarnings };
}
...
function normalizeIdentity(value: string): string {
  return value.normalize("NFKC").trim().toLowerCase().replace(/^https?:\/\//, "").replace(/^www\./, "");
}
